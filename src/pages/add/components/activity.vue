  ...
  <view class="content">
    ...
    <view class="card">
      <view class="button_group">
        <view class="button" @click="goPhotoStore">
          <uni-icons class="icon" type="images" size="15"></uni-icons>
          从图片库选择
        </view>
        <view class="button" @click="handleDefaultContent">
...
export default {
  ...
  onLoad() {
    ...
    eventChannel.on("acceptImages", (paths) => {
      //   this.imgList = paths.map((path) => ({ url: path }));
      this.imgList = [...this.imgList, ...paths.map((path) => ({ url: path }))];
    });
  },
  methods: {
    goPhotoStore() {
      uni.navigateTo({
        url: "/pages/add/components/photoStore",
      });
    },
    ...
    handleConfirm(e) {
      this.inputText = e.detail.value.replace(/(\r\n|\n\r|\r)/g, "\n");
    },
    openLocation() {
    ...
    async handleSubmit() {
      ...
      console.log("上传成功", imgs, this.imgList);
      ...
      const res = await this.$api.activity.create({
        ...
        images: imgs
          ...
          .concat(this.imgList.map((item) => item.url)),
      });
